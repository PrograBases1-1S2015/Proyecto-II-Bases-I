...
BEGIN
	...
	FROM species_found, specie, Size, Habitat, Beak_Type, Color, offspring_quantity, Gender, Family, Sub_Order, Orders, Class
        ...
        AND OR p_Size_Name = Size_Name
        AND OR p_Habitat_Name = Habitat_Name
        AND OR p_Beak_Name = Beak_Name
        AND OR p_Color_Name = Color_Name
        AND OR p_Quantity = Quantity
        AND OR p_Gender_Name = Gender_Name
        AND OR p_Family_Name = Family_Name
        AND OR p_Sub_Order_Name = Sub_Order_Name
        AND OR p_Order_Name = Order_Name
        AND OR p_Class_Name = Class_Name
END
